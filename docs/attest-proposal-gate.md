...
> last 40 non-merge commits on `main` (at `332a40b`).
>
> **§2.1 is ADR-0055 and is implemented.** Gating it produced two entries this file did not
> plan: **ADR-0056** (the log's fourth and fifth relations, which were in use and undocumented
> — a false claim in ADR-0055 is what exposed them) and **ADR-0057** (an entry becomes
> immutable when its commit is *pushed*, which nothing had ever said). Both came out of passes
> that ran *beside* the commits rather than before them, which is §2.4's argument made twice in
> one session.
>
> **Everything else here is undecided.** §2.2–2.4 change no file yet; the two entries in §5
> are `DRAFT` and carry **no id** on purpose, because two attempts to reserve one were
> invalidated within the hour; the change list in §4 is a plan and not a diff; the seven
> questions in §6 are open. Delete this file once B and C are decided — what survives it is
> the ADR log.
...
### 2.1 The finish line — **recorded as ADR-0055 and implemented**
...
| `docs/attest-decisions.md` | the phase-B entry (Draft B below) | B |
...
| `docs/attest-decisions.md` | the phase-C entry (Draft C below) | C |
| `.attest/gate-*.md` | no change to any existing record (append-only); new records carry the new lines | — |

## 5. ADR drafts

The drafts below carry **no id**: two attempts to reserve one were invalidated within the hour
by entries this proposal's own gate runs produced. An id is assigned when `/decision`
records the entry, at which point the status becomes `Accepted` and the date is the day it is
written.
...
## Draft B — a pass runs only when the diff touched its ground; `full` runs them all · DRAFT
...
## Draft C — `/gate fix` loops only on what a command can verify, at most three times · DRAFT
...
7. **Should the ship guard block on a major, not only a blocker?** Surfaced by the `/decision`
   ...
   commit time. ADR-0037 never weighed `major`, so the gap predates the rule and was never
   deliberated. The asymmetry is defensible — the two gates ask different questions — and it is
   now flagged in the ladder as an open question rather than left for the next reader to find.
   Proposed: leave it, and revisit only with evidence of a major that should have stopped a push.
