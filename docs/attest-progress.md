...
be followed — and the one outside git no longer claims *"not a git checkout"*, which git also says
of a repository it refuses to read. Suite **367 → 385**: seventeen cases, eleven failing against
`main`, plus one ship record. Without Git for Windows the guard still does not run at all — the
named limit.
...
- **From the full gate on the kit-0.12.0 branch (`gate-20260924-090400-f5134a9`) — the nits,
  ...
  take an edit; two stale *"once for `Bash`"* comments went with them.
  - `smoke.sh`'s `T4d` fixture is byte-identical to `main`'s `settings.json`, so its two cases
    fail there as *identical*, not for the reason they test; `T4e` covers the real case. A
    `_note` line, as `T4b` has, would make `T4d` say what it means.
  - After `git checkout --orphan` in a repository with commits, the guard says *"this repository
    has no commits yet"*; the advice is right, the noun is not — *"this branch"*.
  - The `not-a-repo` case assumes `$WORK` is outside every git tree; a `TMPDIR` inside a dotfiles
    `$HOME` would flip it. `GIT_CEILING_DIRECTORIES="$WORK"` on that call closes it.
  - Outside this branch: GUIDE 2.2 still says the guard *"matches the sha in the record's
    name"*; since ADR-0050 it matches the record's `HEAD:` line.
